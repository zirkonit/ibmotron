from __future__ import annotations

import json
import shutil
from collections import Counter, defaultdict
from pathlib import Path
from typing import Any, Iterable

from ibm650_it.dataset.io import load_jsonl, resolve_record_base, resolve_record_path, write_jsonl


REVIEW_CATEGORY_ORDER = [
    "partial_dictionary_tail_truncation",
    "final_a_dictionary_drop",
    "tail_only_other",
    "candidate_missing",
    "non_tail_divergence",
    "malformed_pit",
    "functional_success_exact_failure",
]

REPRESENTATIVE_PLAN = [
    ("partial_dictionary_tail_truncation", 4),
    ("final_a_dictionary_drop", 2),
    ("tail_only_other", 2),
    ("candidate_missing", 1),
]


def _read_cards(path: Path) -> list[str]:
    return path.read_text(encoding="latin-1").splitlines()


def _diff_indices(reference_cards: list[str], candidate_cards: list[str]) -> list[int]:
    diff_indices: list[int] = []
    max_len = max(len(reference_cards), len(candidate_cards))
    for index in range(max_len):
        reference_card = reference_cards[index] if index < len(reference_cards) else None
        candidate_card = candidate_cards[index] if index < len(candidate_cards) else None
        if reference_card != candidate_card:
            diff_indices.append(index)
    return diff_indices


def _strip_leading_space_first_card_diff(
    diff_indices: list[int],
    reference_cards: list[str],
    candidate_cards: list[str],
) -> list[int]:
    filtered: list[int] = []
    for index in diff_indices:
        if index == 0 and reference_cards and candidate_cards:
            if reference_cards[0].lstrip() == candidate_cards[0].lstrip():
                continue
        filtered.append(index)
    return filtered


def _copy_if_exists(source: Path | None, destination: Path) -> None:
    if source is None or not source.exists():
        return
    destination.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(source, destination)


def _case_sort_key(case: dict[str, Any]) -> tuple[float, int, float, str]:
    return (
        float(case["metrics"]["normalized_edit_distance"]),
        int(case["diff_count"]),
        1.0 - float(case["metrics"]["per_card_exact"]),
        str(case["id"]),
    )


def _is_partial_dictionary_tail(last_line: str) -> bool:
    stripped = last_line.strip()
    if not stripped:
        return False
    return (
        stripped.endswith("s00")
        or stripped.endswith("s0")
        or stripped.endswith(" 00")
        or stripped.endswith("i")
        or " u0" in stripped
        or stripped == "3         s"
    )


def _classify_review_category(
    *,
    prediction: dict[str, Any],
    reference_cards: list[str],
    candidate_cards: list[str],
) -> str:
    if prediction.get("functional") and not prediction.get("metrics", {}).get("exact_match"):
        return "functional_success_exact_failure"
    if not prediction.get("assemblable", False):
        return "malformed_pit"

    diff_indices = _strip_leading_space_first_card_diff(
        _diff_indices(reference_cards, candidate_cards),
        reference_cards,
        candidate_cards,
    )
    if not diff_indices:
        return "functional_success_exact_failure"

    max_len = max(len(reference_cards), len(candidate_cards))
    tail_only = all(index >= max_len - 12 for index in diff_indices)
    if not tail_only:
        return "non_tail_divergence"

    last_line = candidate_cards[-1] if candidate_cards else ""
    if _is_partial_dictionary_tail(last_line):
        return "partial_dictionary_tail_truncation"
    if last_line.strip().startswith("4         a") or last_line.strip().startswith("3         a"):
        return "final_a_dictionary_drop"
    return "tail_only_other"


def _archive_case(
    *,
    case: dict[str, Any],
    archive_root: Path,
    ordinal: int,
    reference_index: Path,
    prediction_index: Path,
) -> dict[str, Any]:
    case_dir = archive_root / f"{ordinal:04d}_{case['id']}"
    case_dir.mkdir(parents=True, exist_ok=True)
    reference_base = resolve_record_base(reference_index)
    prediction_base = prediction_index.parent

    reference = case["reference_record"]
    prediction = case["prediction_record"]
    source_path = resolve_record_path(str(reference["source"]["it_text_v1"]), reference_base)
    reference_pit = resolve_record_path(str(reference["reference"]["translate"]["pit_raw_canonical"]), reference_base)
    candidate_pit = resolve_record_path(str(prediction["pit_raw_canonical"]), prediction_base)

    _copy_if_exists(source_path, case_dir / "source.it")
    _copy_if_exists(reference_pit, case_dir / "reference_pit.dck")
    _copy_if_exists(candidate_pit, case_dir / "candidate_pit.dck")
    for target, name in [
        (prediction.get("assemble", {}).get("console_log"), "assemble_console.log"),
        (prediction.get("assemble", {}).get("stdout_log"), "assemble_stdout.log"),
        (prediction.get("assemble", {}).get("print_log"), "assemble_print.log"),
        (prediction.get("run", {}).get("console_log"), "run_console.log"),
        (prediction.get("run", {}).get("stdout_log"), "run_stdout.log"),
        (prediction.get("run", {}).get("print_log"), "run_print.log"),
    ]:
        if target:
            _copy_if_exists(resolve_record_path(str(target), prediction_base), case_dir / name)

    summary = {
        "id": case["id"],
        "band": case["band"],
        "review_category": case["review_category"],
        "failure_type": case["failure_type"],
        "metrics": case["metrics"],
        "diff_count": case["diff_count"],
        "first_diff_index": case["first_diff_index"],
        "candidate_lines": case["candidate_lines"],
        "reference_lines": case["reference_lines"],
        "functional": case["functional"],
        "assemblable": case["assemblable"],
    }
    (case_dir / "summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    return summary


def _parse_bands(bands: Iterable[str]) -> list[str]:
    parsed = [band.strip().upper() for band in bands if band.strip()]
    if not parsed:
        raise ValueError("at least one band is required")
    return parsed


def build_band_failure_review(
    *,
    reference_index: Path,
    prediction_index: Path,
    output_root: Path,
    bands: Iterable[str],
) -> dict[str, Any]:
    selected_bands = _parse_bands(bands)
    output_root.mkdir(parents=True, exist_ok=True)
    reference_records = {str(record["id"]): record for record in load_jsonl(reference_index)}
    predictions = load_jsonl(prediction_index)
    reference_base = resolve_record_base(reference_index)
    prediction_base = prediction_index.parent

    cases: list[dict[str, Any]] = []
    by_band_category: dict[str, Counter[str]] = defaultdict(Counter)
    feature_counts: dict[str, Counter[str]] = defaultdict(Counter)
    for prediction in predictions:
        band = str(prediction.get("band"))
        if band not in selected_bands:
            continue
        if prediction.get("failure_type") == "exact_match":
            continue
        reference = reference_records.get(str(prediction["id"]))
        if reference is None:
            continue
        for feature in reference.get("generator", {}).get("features", []):
            feature_counts[band][str(feature)] += 1

        candidate_pit = resolve_record_path(str(prediction["pit_raw_canonical"]), prediction_base)
        if not candidate_pit.exists():
            review_category = "candidate_missing"
            diff_indices: list[int] = []
            candidate_lines = 0
            reference_lines = 0
        else:
            reference_pit = resolve_record_path(str(reference["reference"]["translate"]["pit_raw_canonical"]), reference_base)
            reference_cards = _read_cards(reference_pit)
            candidate_cards = _read_cards(candidate_pit)
            diff_indices = _strip_leading_space_first_card_diff(
                _diff_indices(reference_cards, candidate_cards),
                reference_cards,
                candidate_cards,
            )
            review_category = _classify_review_category(
                prediction=prediction,
                reference_cards=reference_cards,
                candidate_cards=candidate_cards,
            )
            candidate_lines = len(candidate_cards)
            reference_lines = len(reference_cards)

        by_band_category[band][review_category] += 1
        cases.append(
            {
                "id": prediction["id"],
                "band": band,
                "review_category": review_category,
                "failure_type": prediction.get("failure_type"),
                "metrics": prediction.get("metrics", {}),
                "assemblable": prediction.get("assemblable", False),
                "functional": prediction.get("functional", False),
                "diff_count": len(diff_indices),
                "first_diff_index": diff_indices[0] if diff_indices else None,
                "candidate_lines": candidate_lines,
                "reference_lines": reference_lines,
                "prediction_record": prediction,
                "reference_record": reference,
            }
        )

    cases_output = [
        {
            key: value
            for key, value in case.items()
            if key not in {"prediction_record", "reference_record"}
        }
        for case in sorted(cases, key=_case_sort_key, reverse=True)
    ]
    write_jsonl(output_root / "cases.jsonl", cases_output)

    selected: list[dict[str, Any]] = []
    selected_ids: set[str] = set()
    for band in selected_bands:
        band_cases = sorted(
            [case for case in cases if case["band"] == band],
            key=_case_sort_key,
            reverse=True,
        )
        for case in band_cases[: min(4, len(band_cases))]:
            selected.append(case)
            selected_ids.add(str(case["id"]))

    for category, target_count in REPRESENTATIVE_PLAN:
        category_cases = sorted(
            [case for case in cases if case["review_category"] == category],
            key=_case_sort_key,
            reverse=True,
        )
        added = 0
        for case in category_cases:
            if str(case["id"]) in selected_ids:
                continue
            selected.append(case)
            selected_ids.add(str(case["id"]))
            added += 1
            if added >= target_count:
                break

    remaining = sorted(
        [case for case in cases if str(case["id"]) not in selected_ids],
        key=_case_sort_key,
        reverse=True,
    )
    for case in remaining[: max(0, 12 - len(selected))]:
        selected.append(case)
        selected_ids.add(str(case["id"]))

    archive_root = output_root / "selected_failures"
    if archive_root.exists():
        shutil.rmtree(archive_root)
    archive_root.mkdir(parents=True, exist_ok=True)
    archived = [
        _archive_case(
            case=case,
            archive_root=archive_root,
            ordinal=index,
            reference_index=reference_index,
            prediction_index=prediction_index,
        )
        for index, case in enumerate(sorted(selected, key=_case_sort_key, reverse=True), start=1)
    ]

    category_totals: Counter[str] = Counter()
    for counter in by_band_category.values():
        category_totals.update(counter)

    recommendations = [
        "Upweight B2 and B3 during SFT preparation so the model sees more control-flow and iterate tails per epoch.",
        "Expand B2 into multiple control-flow families with different statement counts, output arities, and dictionary-tail lengths.",
        "Expand B3 into more iterate families with different indexed targets and post-loop reductions so tail-card lengths vary more aggressively.",
    ]

    summary = {
        "reference_index": str(reference_index),
        "prediction_index": str(prediction_index),
        "bands": selected_bands,
        "non_exact_count": len(cases),
        "category_counts": dict(category_totals),
        "by_band_category_counts": {band: dict(by_band_category[band]) for band in selected_bands},
        "feature_counts": {band: dict(feature_counts[band]) for band in selected_bands},
        "selected_count": len(archived),
        "selected_categories": [entry["review_category"] for entry in archived],
        "recommendations": recommendations,
    }
    (output_root / "summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    lines = [
        f"# {'/'.join(selected_bands)} Failure Review",
        "",
        f"- Non-exact cases: {len(cases)}",
        f"- Selected representative failures: {len(archived)}",
        "",
        "## Category Counts",
    ]
    for category in REVIEW_CATEGORY_ORDER:
        if category in category_totals:
            lines.append(f"- `{category}`: {category_totals[category]}")
    lines.append("")
    lines.append("## By Band")
    for band in selected_bands:
        lines.append(f"- `{band}`:")
        for category in REVIEW_CATEGORY_ORDER:
            count = by_band_category[band].get(category, 0)
            if count:
                lines.append(f"  - `{category}`: {count}")
    lines.append("")
    lines.append("## Feature Footprint")
    for band in selected_bands:
        lines.append(f"- `{band}` features: {dict(feature_counts[band])}")
    lines.append("")
    lines.append("## Recommendations")
    for recommendation in recommendations:
        lines.append(f"- {recommendation}")
    lines.append("")
    lines.append("## Selected Cases")
    for entry in archived:
        lines.append(
            f"- `{entry['id']}` ({entry['band']}): `{entry['review_category']}` · "
            f"diff_count={entry['diff_count']} · first_diff_index={entry['first_diff_index']} · "
            f"candidate_lines={entry['candidate_lines']} · reference_lines={entry['reference_lines']}"
        )
    (output_root / "review.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return summary
